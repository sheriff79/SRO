...
Функция ПолучитьМассивПериодов(Период)	
	
	МассивПериодов = НОвый Массив;
	ТекПериод = НачалоДня(КонецКвартала(Период.ДатаНачала));
	
	Пока Истина = Истина Цикл
		
		Если ТекПериод <= НачалоДня(Период.ДатаОкончания) Тогда
...
Процедура СформироватьНаСервере()
	...
	Если ЗначениеЗаполнено(КПК) Тогда
		МассивКПК.Добавить(КПК);
	КонецЕсли;
	
	СтруктураПараметров = НОвый Структура;
	СтруктураПараметров.Вставить("МассивКПК", МассивКПК);
	СтруктураПараметров.Вставить("ДатаНачалаОтчета", Период.ДатаНачала);
	СтруктураПараметров.Вставить("ДатаОкончанияОтчета", Период.ДатаОкончания);
	
	ТабДокумент =  Отчеты.СРО_ТаблицаФинансовыхПоказателейКПК.СформироватьТабличныеДокументы(СтруктураПараметров); 	

	
	//         СРО_ТаблицаФинансовыхПоказателейКПК
	//
	...
	//	
	//МассивПериодов = ПолучитьМассивПериодов(Период);
	//
	...
	//	
	//	Если ДокументОтчетности.ТаблицаПоказателей.Количество() = 0 Тогда
	//		Продолжить;
	//	КонецЕсли;
	//						
	//	СрокДеятельностиКПК = (НачалоДня(Период.ДатаОкончания) - НачалоДня(ДокументОтчетности.КПКДатаСоздания)) / 86400;
	//	КодДокумента = ДокументОтчетности.КодДокумента;
	//	
	//	СтруктураЗначение = Новый Структура;
	//	Для Каждого СтрокаПараметры ИЗ ДокументОтчетности.ТаблицаПоказателей Цикл
	...
	//	КонецЦикла;  					
	//	
	//	ЗначенияНормативов = СРО_ЗагрузкаОтчетности.ПолучитьНормативы_РасчетныеИПоБазе(СтруктураЗначение, ДокументОтчетности.КодДокумента); 		
	//	
	//	// ФН1
	...
	//	НоваяСтрока.ФН = "ФН1";
	//	НоваяСтрока.Период = Выборка.ДатаОкончанияОтчетногоПериода;
	...
	//Результат.Вывести(ОбластьЗаголовок); 	
	//
	//Результат.Вывести(ОбластьШапка2ПериодИсточник); 
	//
	//
	//Для Каждого ЭлементФН ИЗ МассивФН Цикл  	
	//	ОбластьШапка2ФН.Параметры.ФН = ЭлементФН;	
	//	Результат.Присоединить(ОбластьШапка2ФН); 	
	//КонецЦикла;        	
	//
	...
	//	
	//	Результат.Вывести(ОбластьСтрока2ПериодИсточник); 	 
	//	
	//	ФН1 = 0;
	//	ФН2 = 0;
	//	ФН3 = 0;
	...
	//	КонецЕсли;
	//	
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН1);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 	 
	//	
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН2);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 	 
	//	
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН3);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 
	//			
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН4);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 
	//	
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН5);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 
	//	
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН6);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 
	//	
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН7);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 
	//			
	//	ОбластьСтрока2ФН.Параметры.ФН = ФорматироватьЧисло(ФН8);
	//	Результат.Присоединить(ОбластьСтрока2ФН); 						
	//	
	//КонецЦикла;	
	
	Результат.Вывести(ТабДокумент[0]);
КонецПроцедуры

&НаСервереБезКонтекста
Функция ФорматироватьЧисло(ФН)
...
&НаКлиенте
Процедура СформироватьДляРассылки(Параметр1, Параметр2 = Неопределено, Параметр3 = Неопределено)
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "Запущена СФОРМИРОВАТЬ ДЛЛЯ РАССЫЛКИ";
	Сообщение.Сообщить();
	
	
КонецПроцедуры

Процедура ВыполнитьКоманду(Параметр1, Параметр2 = Неопределено, Параметр3 = Неопределено)
	
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "Запущена ВЫПОЛНЕНА КОМАНДА";
	Сообщение.Сообщить();
...
Процедура СформироватьОтчет(Параметр1, Параметр2 = Неопределено, Параметр3 = Неопределено)
	
	Сообщение = Новый СообщениеПользователю;
	Сообщение.Текст = "Запущена СФОРМИРОВАТЬ ОТЧЕТ";
	Сообщение.Сообщить();
